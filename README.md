...
| 层 | 解决的问题 | 做法 |
|---|---|---|
| **归集口径** | 抽出来全是噪音 | 只认三类显式信号；长期档案只取显式标记；自己写下的待办直接进清单 |
| **防误收兜底** | 抽错了怎么办 | 候选制按来源分级（只拦从正文推断的）+ 独立判定档案 + 分级模糊匹配 |
| **归属补全** | 看不出是哪条线的 | 五路信号加权打分；**项目锚**回溯抗标题更替；分错了可在页面「改组」手工纠正 |
...
| `todo-state.json` | **人工判定档案**（勾选 / 忽略 / 归属调整记录，与清单解耦） | **别删**——你的判定都在这里 |
...
# 在页面里勾选、或点条目右侧「改组」调整归属 → 点「导出勾选结果」得到 checked.json
# 再把结果写回清单与判定档案：
